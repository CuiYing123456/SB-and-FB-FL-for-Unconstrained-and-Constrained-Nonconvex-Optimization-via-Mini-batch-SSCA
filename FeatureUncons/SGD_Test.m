...
T=1000;
B_list = [10, 100, 1000];
lr_init_list = [0.1, 0.3, 1];  % [0.1, 0.3, 1] --> opt: [1, 1, 1] (OK!)
dim_list = [0.1, 0.3, 0.5];  % [0.1, 0.3, 0.5] --> opt: [0.1, 0.1, 0.1] (OK!)
for i = 1 : length(B_list)
    ...
    for j = 1 : length(lr_init_list)
        ...
        for k = 1 : length(dim_list)
            ...
            save(sprintf('./SaveData/FUa2_B%d_lrinit%0.1f_dim%0.1f.mat', B, lr_init, dim), 'a_end2');
            save(sprintf('./SaveData/FUc2_B%d_lrinit%0.1f_dim%0.1f.mat', B, lr_init, dim), 'c_end2');
        end
    end
end
